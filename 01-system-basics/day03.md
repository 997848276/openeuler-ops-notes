...
1.  **“你排查过什么性能问题？”** &#8203;
   → 主机名解析慢 5.2 秒：/etc/hosts 缺映射导致绕过本地表问 DNS，
   其中 8.8.8.8 不可达触发超时。修复后 0.035s，提速 150 倍。

2.  **“你做过安全加固吗？”** &#8203;
   → SSH 加固：先建密钥通道验证可用，再关闭密码认证。
   配置改动前后对比 + 双向验证（密钥通 / 密码拒）。
   关键细节：chmod 700/600、sshd -t 语法检查、用 reload 而非 restart。

3.  **“改配置出过事故吗？”** &#8203;
   → 没有，因为我遵循：**先备份（.bak）→ 改 → 语法检查 → 平滑重载 → 验证**。
   每次改 SSH 前都保留一个旧会话窗口作为回退通道。
